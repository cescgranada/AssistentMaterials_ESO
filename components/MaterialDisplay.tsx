...
import { BookOpen, CheckCircle, Accessibility, Eye, Copy, Check, ClipboardList, FileText, Printer, Code2, Terminal, HelpCircle, X, Home, Edit3 } from 'lucide-react';
...
export const MaterialDisplay: React.FC<MaterialDisplayProps> = ({ content, onReset, onBackToEdit }) => {
      ...
      <div className="mb-6 flex flex-col gap-4 no-print">
        ...
        <div className="w-full overflow-x-auto pb-2 scrollbar-hide">
          <div className="flex flex-nowrap items-center gap-1 bg-white p-1.5 rounded-[1.5rem] border border-slate-200 shadow-xl min-w-max">
            ...
            <ActionButton onClick={() => handleDownload('md')} icon={<FileText className="w-4 h-4" />} label="[📄 Word/MD]" />
            <ActionButton onClick={() => handleDownload('tex')} icon={<Code2 className="w-4 h-4" />} label="[🧬 Overleaf]" />
            <ActionButton onClick={() => handleDownload('ipynb')} icon={<Terminal className="w-4 h-4" />} label="[🐍 Colab]" />
            <ActionButton onClick={() => window.print()} icon={<Printer className="w-4 h-4" />} label="[🖨️ PDF]" highlight />
          </div>
        </div>
      </div>
