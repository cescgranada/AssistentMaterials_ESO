...
export const InputForm: React.FC<InputFormProps> = ({ onSubmit, isGenerating, initialParams, initialStep, onStepChange }) => {
  ...
  const handleStartAnalysis = async () => {
    if (!params.subject || (!fileContent && !params.manualDescription)) {
      alert("Tria matèria i puja un document o descriu el tema.");
      return;
    }
    setIsAnalyzing(true);
    try {
    ...
    return (
      <div className="w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 animate-fade-in-up">
        ...
        <div className="p-10 space-y-8">
          ...
          <div className="space-y-8 pt-6 border-t border-slate-100">
            <div className="space-y-3">
              ...
              >
                ...
                {fileName && (
                  <button onClick={removeFile} className="absolute top-4 right-4 p-2 bg-white rounded-xl shadow-md text-red-500 hover:bg-red-50 border border-red-100"><Trash2 className="w-4 h-4" /></button>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <textarea 
                ...
                placeholder="Ex: Unitat sobre cinemàtica o un relleu geogràfic..."
          ...
          >
            {isAnalyzing ? <span className="animate-spin text-xl">⏳</span> : <Wand2 className="w-6 h-6" />}
            {isAnalyzing ? 'Analitzant...' : 'Analitzar i Continuar'}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full max-w-5xl space-y-8 animate-fade-in-up pb-20 px-4">
        ...
        {params.topics.map((topic, idx) => (
          <div key={topic.id} className={`bg-white rounded-[3rem] shadow-2xl border-2 transition-all flex flex-col md:flex-row overflow-hidden ${!topic.isIncluded ? 'opacity-40 grayscale border-transparent' : 'border-white'}`}>
            <div className="p-10 md:w-1/3 border-r border-slate-50 bg-blue-50/10">
              ...
              <h4 className="text-2xl font-black text-slate-900 mb-4 leading-tight">{topic.title}</h4>
              ...
              {topic.isIncluded && (
                <button 
                  ...
                  className={`w-full mt-8 p-4 rounded-2xl flex items-center justify-center gap-3 transition-all border-2 font-black text-[10px] uppercase tracking-[0.2em] shadow-lg active:scale-95 ${topic.isAdapted ? 'bg-blue-700 border-blue-800 text-white' : 'bg-white border-blue-100 text-blue-600 hover:bg-blue-50'}`}
                >
                  <Accessibility className="w-5 h-5" />
                  {topic.isAdapted ? 'Adaptació DUA Activa' : 'Activar Adaptació DUA'}
                </button>
              )}
            </div>
              ...
              {topic.isIncluded && (
                <>
                  <div className="space-y-4">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] block">Nivell de Teoria</label>
      ...
      <div className="flex justify-center pt-12">
        <button onClick={() => onSubmit(params)} disabled={isGenerating || !params.topics.some(t => t.isIncluded)} className="w-full max-w-2xl py-8 rounded-[3rem] bg-blue-700 text-white text-2xl font-black shadow-2xl hover:bg-blue-800 transition-all flex items-center justify-center gap-6 active:scale-95 disabled:bg-slate-200 group uppercase tracking-[0.3em]">
          {isGenerating ? <span className="animate-spin text-3xl">⏳</span> : <CheckCircle className="w-10 h-10 group-hover:scale-110 transition-transform" />}
